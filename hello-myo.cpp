...
public:
    ...
    void onOrientationData(myo::Myo* myo, uint64_t timestamp, const myo::Quaternion<float>& quat)
    {
		...
		if (recognizeWave(myo,timestamp))
		{
			
			
			//PlaySound("hello1.wav", NULL, SND_FILENAME);
			//indicates waved
	...
	/* Code for sending test, API not working Twilio
	void sendText()
	{
	...
	*/
	...
	bool recognizeWave(myo::Myo* myo, uint64_t timestamp)
	{

		int x = timestamp;
		//std::cout << x << "\n";
		values1[counter] = yaw_w;
		values2[counter] = pitch_w;
		counter++;
		if (!over)
		{
			counter2++;
		...
		if ((abs(x) % 1000 <= 20) && (over))
		{
			int small1 = values1[0];
			int small2 = values2[0];
			...
			if (abs(big1 - small1) >= 3)
			{
				
				if ((abs(big2 - small2) >= 1) && (abs(big2 - small2) <= 4))
				{
					
					if ((abs(big2 - small2) == 5) && (half))
					{
					...
					else if (((abs(big2 - small2) == 5) && (!half)))
					{
		...
		/*
		if (abs(yaw_w - oldYaw) >= 2)
		{
			if ((abs(pitch_w - oldPitch) >= 1) && (abs(pitch_w - oldPitch) <= 4))
			{
